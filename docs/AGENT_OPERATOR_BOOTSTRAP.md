...
This is the one-time setup required before agents can stop relying on the founder as a command relay.
...
The Cloudflare token for this phase should be scoped to the account that owns `ventureos-public` and should receive only the permission required to edit/deploy Workers. Do not grant DNS, Access, billing, account-administration or unrelated zone permissions to this token.

The reviewed workflow hardcodes the target Worker name and does not mutate Custom Domains or Access configuration.

Create a repository or environment variable:

`VENTUREOS_PROGRESS_DEPLOY_ENABLED=false`

Keep it `false` while validating the workflow and credentials. Change it to `true` only after the first dry review and after confirming that `progress.ventureos.site` remains correctly protected/routed.
...
The target operating model is that progress-only static deployments can become automatic after merge, while sensitive environments keep explicit review gates.

## 2. Cloudflare Access for confidential command center

The confidential Founder Command Center must be protected before its private reports/state are deployed.
...
- public `workers.dev` and preview bypasses are disabled or equivalently protected before confidential assets are introduced;
...
- direct machine-generated daily state updates may be introduced later only with explicit reviewed rules;
...
Pi does not need the raw Cloudflare token if GitHub Actions performs deployment. Pi needs only enough GitHub authority to:
...
Create a separate deployment principal only when the reviewed VPS operator design is ready. Prefer:
...
Existing private-staging deployment workflows remain authoritative until a reviewed replacement is merged.
...
1. workflow exists on protected `main`;
2. CI is green;
3. target Worker is still `ventureos-public`;
...
After these checks, routine progress-site deployment can become agent-driven rather than founder-relayed.