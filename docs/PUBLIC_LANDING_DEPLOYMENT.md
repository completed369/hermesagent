# VentureOS public landing deployment

## Purpose

This runbook covers the dedicated static public marketing/progress page for
`https://ventureos.site`.

The public artifact itself lives in `deploy/public-landing/` and is intentionally
limited to static HTML/CSS plus Cloudflare Pages response-header configuration.
It does not include the private VentureOS application, authenticated screens,
backend APIs, database, Temporal worker, storage, analytics, external scripts,
external fonts, cookies, or provider integrations.
...
7. Upload only the contents of `deploy/public-landing/` for the reviewed public
   landing artifact. That directory must contain only `index.html`, `styles.css`,
   `404.html`, and `_headers`.
