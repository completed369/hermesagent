...
for (const path of files) {
  ...
  if (extname(path) === '.js') {
    ...
    if (
      /runtime-process-tree-fixture|deterministic-supervision|native-supervisor-helper|native-supervisor-addon|native-runtime-fixture|authenticated-lifecycle-addon|authenticated-supervised-lifecycle|retained-pidfd-recovery|retained-native-local-ipc-addon|retained-native-listener-lifecycle-addon|NATIVE_SUPERVISOR_DENIED|AUTHENTICATED_TRANSCRIPT|node:child_process|MC4CAQAwBQYDK2VwBCIE/u.test(
        source,
      )
    )
