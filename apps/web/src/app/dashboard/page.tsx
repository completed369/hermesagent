    ...
    <div style={{ display: 'grid', gap: 20 }}>
      <div>
        ...
        <p style={{ color: 'var(--vos-text-muted)', margin: '4px 0 0' }}>
          {data ? `${data.workspace.name} · ${data.workspace.baseCurrency} · ${data.memberCount} member(s)` : 'Loading workspace...'}
        </p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 16 }}>
        ...
        <div className="vos-card">
          <p style={{ fontSize: 12, color: 'var(--vos-text-muted)', margin: 0 }}>Pending approvals</p>
          <p style={{ fontSize: 28, fontWeight: 700, margin: '6px 0 0' }}>0</p>
          <span className="vos-badge vos-badge--mock">Phase 3+</span>
        </div>
        <div className="vos-card">
          <p style={{ fontSize: 12, color: 'var(--vos-text-muted)', margin: 0 }}>Budget utilisation (month)</p>
          <p style={{ fontSize: 28, fontWeight: 700, margin: '6px 0 0' }}>€0 / €100</p>
          <span className="vos-badge vos-badge--mock">Phase 7</span>
        </div>
        <div className="vos-card">
          <p style={{ fontSize: 12, color: 'var(--vos-text-muted)', margin: 0 }}>Integrations connected</p>
          <p style={{ fontSize: 28, fontWeight: 700, margin: '6px 0 0' }}>
            {data ? data.integrations.filter((i) => i.status === 'CONNECTED').length : '-'} / {data?.integrations.length ?? '-'}
          </p>
          ...
          <tbody>
            {(data?.integrations ?? []).map((i) => (
              <tr key={i.provider} style={{ borderTop: '1px solid var(--vos-border)' }}>
                ...
                <td>
                  <span className={`vos-badge ${i.status === 'CONNECTED' ? 'vos-badge--ok' : 'vos-badge--mock'}`}>
                    {i.status}
                  </span>
                </td>
              </tr>
      ...
      <div className="vos-card">
        ...
        <p style={{ fontSize: 13, color: 'var(--vos-text-muted)', margin: 0 }}>
          This Phase 1 build implements founder authentication, workspace + RBAC, founder onboarding,
          the audit and security event trails, and integration/health status shown above - all backed
          by a real PostgreSQL database. Opportunity research, the AI board of agents, approvals,
          product/listing studios and finance dashboards are architected (see docs/ROADMAP.md) but not
          yet built; their nav items above are greyed out and labelled by phase.
        </p>
      </div>
    </div>
  );
